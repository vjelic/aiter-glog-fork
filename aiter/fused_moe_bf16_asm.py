...
def asm_moe(
    ...
    if fc1_scale is None:
        ...
        if (
            w1.dtype in [torch.float8_e4m3fnuz, torch.int8]
        ...
        assert w1.dtype == torch.float8_e4m3fnuz, (
            "asm_moe for block_scale only support float8_e4m3fnuz weight"
        )
        assert w2.shape[2] * 2 == w1.shape[1], (
        ...
        a8_type = (
            w1.dtype
            if w1.dtype != torch.int32 and w1.dtype != torch.uint32
