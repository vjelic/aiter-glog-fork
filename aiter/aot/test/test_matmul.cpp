...
float random_float() { return static_cast<float>(rand()) / static_cast<float>(RAND_MAX); }
    ...
    for(int M = 16; M <= 64; M *= 2)
    {
        for(int N = 16; N <= 64; N *= 2)
        {
            for(int K = 16; K <= 32; K *= 2)
            {
                ...
                ASSERT_EQ(hipSuccess,
                          matmul_fp16_b3a5a34c_0d1d2d34567c89c10d11c121314(
                ...
                ASSERT_EQ(
                    hipSuccess,
                    hipMemcpy(h_C.data(), d_C, M * N * sizeof(__half), hipMemcpyDeviceToHost));
