...
@triton.jit
def _gemm_a8w8_blockscale_splitk_kernel(
...
#        pid_n = (pid % num_pid_in_group) // group_size_m
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        ks_start = pid_k * SPLITK_BLOCK_SIZE
        offs_ks = ks_start // GROUP_K
...
def gemm_a8w8_blockscale_splitk(
    x: torch.Tensor,
    w: torch.Tensor,
    ...
    **scale_n = (N + scale_block_size_n - 1) // scale_block_size_n
    ...
    _gemm_a8w8_blockscale_splitk_kernel[grid](
        x,
        w,
        y_temp,
        x_scale,
