...
def generate_bshd_tensor(
    BATCH, SEQ_LEN, NUM_HEADS, D_HEAD, dtype, device="cuda", DEBUG_INPUT=False
):
    # save fp8 type
    is_fp8_dtype = is_dtype_fp8(dtype)
    if is_fp8_dtype:
...
def generate_bhsd_tensor(
    BATCH, NUM_HEADS, SEQ_LEN, D_HEAD, dtype, device="cuda", DEBUG_INPUT=False
):
    # save fp8 type
    is_fp8_dtype = is_dtype_fp8(dtype)
    if is_fp8_dtype:
    ...
    if is_fp8_dtype:
        ...
        x, descale_x = cast_to_fp8(
            x, og_fp8_dtype, "bhsd"
        )  # FIXME: I don't the casting fn supports this atm
        x.requires_grad_()
        return x, descale_x
    else:
        x.requires_grad_()
