...
def find_seq_idx(
    ...
    while left < right:
        mid = (left + right) // 2
        val = tl.load(cu_seqlens_q + mid)
        mid_val = val // BLOCK_M + mid

        if mid_val <= global_seq_idx:
            left = mid + 1
        else:
            right = mid
...
def _attn_fwd_persistent_dynamic(
    ...
    while tile_id < num_tiles:
        continue_condition = True
        next_tile_id = tl.atomic_add(pid_counter, 1)
        off_q_head = tile_id % NUM_Q_HEADS
        off_q_head = remap_xcd(off_q_head, NUM_Q_HEADS, NUM_XCD)
        global_q_index = (tile_id // NUM_Q_HEADS) % total_num_q_blocks
        ...
        cur_batch_query_len = cur_batch_end_idx - cur_batch_start_idx

        # if start_m * BLOCK_M >= cur_batch_query_len:
        #     return
        ...
        if pooled: # NUM_XCDS pools of tiles (pids). The pool the next tile gets fetched from is based on which XCD the persistent workgroup exists.
            ...
            _attn_fwd_persistent_dynamic[grid](
                ...
                *o_strides,
                ...
                philox_offset,
                SEQLEN_Q=max_seqlen_q,
                total_num_q_blocks=q.shape[0] // config["BLOCK_M"] + batch,
                SEQLEN_K=max_seqlen_k,
                IS_CAUSAL=causal,
                NUM_Q_HEADS=num_q_heads,
                NUM_K_HEADS=num_k_heads,
