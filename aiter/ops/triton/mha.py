...
from aiter.ops.triton.utils.pid_preprocessing import remap_xcd, get_xcd_pool_size, get_xcd_pool_offset
...
def _attn_fwd_persistent_pooled(
    ...
    xcd = 0
    pool_size = num_tiles # get_xcd_pool_size(num_tiles, xcd, NUM_XCD)
    pool_offset = 0 # get_xcd_pool_offset(num_tiles, xcd, NUM_XCD)

    pool_ptr = pool_counters + xcd

    TILE_GROUP_SIZE: tl.constexpr = 16

    # fetch the initial group of tiles from the pool
    tile_id_local = tl.atomic_add(pool_ptr, TILE_GROUP_SIZE)

    while tile_id_local < pool_size:

        # process the fetched group of tiles
        for i in tl.range(0, TILE_GROUP_SIZE, flatten=True):
            
            if (tile_id_local + i) < pool_size:
                tile_id = tile_id_local + i + pool_offset
            else:
                tile_id = pool_offset
        ...
        # fetch the next group of tiles from the pool
        tile_id_local = tl.atomic_add(pool_ptr, TILE_GROUP_SIZE)
...
def _attn_fwd_persistent_static(
    ...
    for i in tl.range(0, high_tiles_per_workgroup):
    ...
    for i in tl.range(low_tiles_per_workgroup, flatten=True):
...
@triton.jit
def _attn_fwd_persistent_dynamic(
    q_ptr: torch.Tensor,
    k_ptr: torch.Tensor,
    v_ptr: torch.Tensor,
    descale_q_ptr: torch.Tensor,
...
def _flash_attn_forward(
    ...
    if persistent:
        ...
        num_tiles_per_seq = triton.cdiv(max_seqlen_q, config["BLOCK_M"])
        num_tiles = batch * num_q_heads * num_tiles_per_seq
        # print("num_tiles", num_tiles)
        grid = (min(num_tiles, NUM_WGS),)
        NUM_XCDS = 8
        
        pooled = False
        dynamic = True # only in effect if pooled False
        ...
        if pooled: # NUM_XCDS pools of tiles (pids). The pool the next tile gets fetched from is based on which XCD the persistent workgroup exists.
            ...
            _attn_fwd_persistent_pooled[grid](
                q,
                k,
                v,
                descale_q,
        ...
        elif dynamic: # single global pool of pids
            pid_counter = torch.ones((1,)).to(q.device).to(torch.int32) * NUM_WGS
            _attn_fwd_persistent_dynamic[grid](
                q,
                k,
                v,
                descale_q,
            ...
            _attn_fwd_persistent_static[grid](
                ...
                *o_strides,
                ...
                USE_INT64_STRIDES=_USE_INT64_STRIDES,
                NUM_WGS=NUM_WGS,
                sorted_tile_indices=tile_work_sorted_indices,
                **config,
            )
