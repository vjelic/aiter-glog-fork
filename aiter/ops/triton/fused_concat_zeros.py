...
def _fused_concat_zeros(
    ...
    x1 = tl.load(
        x1_ptr + pid_m * stride_x1_m + pid_h * stride_x1_h + n1_offs * stride_x1_n
    ).to(out_cat_ptr.type.element_ty)
    x2 = tl.load(
        x2_ptr + pid_m * stride_x2_m + pid_h * stride_x2_h + n2_offs * stride_x2_n
    ).to(out_cat_ptr.type.element_ty)
    tl.store(
        out_cat_ptr
        + pid_m * stride_cat_m
        + pid_h * stride_cat_h
        + n1_offs * stride_cat_n,
        x1,
    )
    tl.store(
        out_cat_ptr
        + pid_m * stride_cat_m
        + pid_h * stride_cat_h
        + (BLOCK_SIZE_D1 + n2_offs) * stride_cat_n,
        x2,
    )
    ...
    tl.store(
        out_zeros_ptr
        + pid_m * stride_zeros_m
        + pid_h * stride_zeros_h
        + n1_offs * stride_zeros_n,
        z,
    )
