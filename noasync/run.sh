...
TRITON_ALWAYS_COMPILE=1 \
  ...
  TRITON_HIP_ASYNC_COPY_BYPASS_PERMUTE=0 \
  TRITON_HIP_ASYNC_FAST_SWIZZLE=0 \
  TRITON_HIP_ASYNC_COPY_OVERLAP=0 \
  TRITON_HIP_GLOBAL_PREFETCH=1 \
  TRITON_HIP_PRESHUFFLE_SCALES=0 \
  python ../op_tests/op_benchmarks/triton/bench_gemm_afp4wfp4.py --shape 128 131072 16384 --metric bandwidth

#head -n 2 results.stats.csv
}
run_mxfp4

#  TRITON_HIP_LOCAL_PREFETCH=0 \
