...
def test_fmoe(
    ...
    #     )
    w1_scale=fp4_utils.e8m0_shuffle(w1_scale)  # e8m0_shuffle will do nothing if it's a fp32
    w2_scale=fp4_utils.e8m0_shuffle(w2_scale)
    ...
    quant_type = qType
    q_dtype_w = w1_qt_aiter.dtype
    q_dtype_a = w1_qt_aiter.dtype if w1_qt_aiter.dtype != torch.uint32 else dtypes.fp8
    ...
    err= checkAllclose(out1_ref, out, msg=f"[perf]\n\
        ck_moe_stage1: num_tokens: {token_num}, E: {E}, topk: {topk}, N: {inter_dim}, K: {model_dim}, {us:>8.2f} us, {token*model_dim*inter_dim*2*topk*2/us/1000/1000:>8.2f} tflops",
    )
