...
def test_reshape_and_cache(
    ...
    checkAllclose(
        k_cache_ref.to(torch.float32)[slots_edit],
        k_cache_a.to(torch.float32)[slots_edit],
        msg=f"k_cache {k_cache_ref.shape}",
    )
    checkAllclose(
        v_cache_ref.to(torch.float32)[slots_edit],
        v_cache_a.to(torch.float32)[slots_edit],
...
test_reshape_and_cache(
    ...
    quantCfg={"y_scale_dtype": torch.float, "quant_dtype": torch.float8_e4m3fnuz},
)
print("\nstart quant fp16->i8")
test_reshape_and_cache(
    4097,
    ...
    torch.float16,
    torch.int8,
    quantCfg={"y_scale_dtype": torch.float, "quant_dtype": torch.int8},
)
print("\nstart quant bf16->i8")
test_reshape_and_cache(
    4097,
