...
def get_moe_configs(dtype: Optional[str]) -> Optional[Dict[int, Any]]:
    ...
    default_configs = {
        "small_M": default_config,
        "medium_M": default_config,
        "large_M": default_config,
    }
    warn(
        f"Intended MoE config file {config_file_path} not found; use default config {default_config=}"
    )
...
def generate_moe_alignment(M, E, top_k, BLOCK_M):
    ...
    sorted_token_ids, expert_ids, num_tokens_post_padded = torch_moe_align_block_size(
        topk_ids, block_size=BLOCK_M, num_experts=E
    )
