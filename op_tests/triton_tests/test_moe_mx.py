...
def test_fused_moe(
    ...
    values = torch.ones((M, E), dtype=torch.float16, device="cuda")
    # softmax_vals = torch.softmax(values, dim=1)
    topk_weights, topk_ids = fused_topk(a_tri, values, 1, False)

    sorted_token_ids, sorted_weights, expert_ids, num_tokens_post_padded, moe_buf = moe_sorting(
        topk_ids, topk_weights, E, 1024, torch.float16, 128
    )
    ...
    if is_b_mixed_input:
        ...
        b_mx_scales = torch.ones_like(b_mx_scales) * 127
        # TODO Add Upcast support
    ...
    print(f"{out2_ck=}")
    print(torch.testing.assert_close(out2_ck.to(fp16_dtype).unsqueeze(1), c_ref.to(fp16_dtype)))

test_fused_moe(1024, 1024, 5120, 1, 8, "mxfp4_e2m1", "mxfp4_e2m1", False, False)