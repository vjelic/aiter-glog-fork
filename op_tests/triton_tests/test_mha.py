...
class TestMHA:
    basic_test_set = {
        "BATCH": [1, 57],
        "SEQLEN_Q_K": [[1, 1], [128, 128], [2, 1], [1, 2]],
        "NUM_Q_K_HEADS":  [[1, 1], [16, 16], [2, 1]],
        "HEAD_SZ": [8, 128],
        "DROPOUT_LSE_SOFTMAX": [[0.2, True, True], [0.0, False, False]], #TODO: Create separate tests for these in this class
        "CAUSAL": [True, False], #TODO Create separate tests for these in this class
        "FP8": [True, False], #TODO Create separate tests for these in this class
    }
    basic_test_set = itertools.product(basic_test_set["BATCH"],
         basic_test_set["SEQLEN_Q_K"],
         basic_test_set["NUM_Q_K_HEADS"],
         basic_test_set["HEAD_SZ"],
         basic_test_set["DROPOUT_LSE_SOFTMAX"],
         basic_test_set["CAUSAL"],
         basic_test_set["FP8"]
    )
    basic_set =  [
        pytest.param(*flatten(test))
        for test in basic_test_set
    ] 
    extended_test_set = {
        "BATCH": [1, 4, 57, 128],
        "SEQLEN_Q_K": [[4, 4], [64, 64], [32, 16], [64, 128]],
        "NUM_Q_K_HEADS":  [[1, 1], [16, 16], [2, 1], [48, 8]],
        "HEAD_SZ": [8, 32, 128],
        "DROPOUT_LSE_SOFTMAX": [[0.2, True, True], [0.0, False, False]], #TODO: Create separate tests for these in this class
        "CAUSAL": [True, False], #TODO Create separate tests for these in this class
        "FP8": [True, False], #TODO Create separate tests for these in this class
    }
    extended_test_set = itertools.product(extended_test_set["BATCH"],
         extended_test_set["SEQLEN_Q_K"],
         extended_test_set["NUM_Q_K_HEADS"],
         extended_test_set["HEAD_SZ"],
         extended_test_set["DROPOUT_LSE_SOFTMAX"],
         extended_test_set["CAUSAL"],
         extended_test_set["FP8"]
    )
    extended_set =  [
    ...
    all_test_params = basic_set + extended_set

    @pytest.mark.parametrize("BATCH, SEQLEN_Q, SEQLEN_K, NUM_Q_HEADS, NUM_K_HEADS, HEAD_SZ, DROPOUT, RETURN_LSE, RETURN_SOFTMAX, CAUSAL, FP8", all_test_params)
    def test_mha(
        self,
        BATCH: int,
        SEQLEN_Q: int,
...
class TestMHAStrides:
    ...
    basic_test_set = {
        ...
        "NUM_Q_K_HEADS":  [[128, 8]],
        "HEAD_SZ": [128],
        "DROPOUT": [[True], [False]], #TODO: Create separate tests for these in this class
        "CAUSAL": [True], #TODO Create separate tests for these in this class
    }
    basic_test_set = itertools.product(basic_test_set["BATCH"],
         basic_test_set["SEQLEN_Q_K"],
         basic_test_set["NUM_Q_K_HEADS"],
    ...
    all_test_params = basic_set

    @pytest.mark.parametrize("BATCH, SEQLEN_Q, SEQLEN_K, NUM_Q_HEADS, NUM_K_HEADS, HEAD_SZ, CAUSAL, DROPOUT", all_test_params)
...
class TestMHABkwd:
    basic_test_set = {
        ...
        "NUM_Q_K_HEADS":  [[1, 1], [16, 16], [2, 1]],
        "HEAD_SZ": [8, 128],
        #"DROPOUT_CAUSAL": [[0.0, True], [0.0, False],[0.2, True], [0.2, False]], #TODO: Create separate tests for these in this class. #Debug Causal + Dropout. fails for seq >= 64
        "DROPOUT_CAUSAL": [[0.0, True], [0.0, False],[0.2, False]], #TODO: Create separate tests for these in this class. #Debug Causal + Dropout. fails for seq >= 64
        #"FP8": [True, False], #TODO Create separate tests for these in this class #TODO Debug FP8
        "FP8": [False], #TODO Create separate tests for these in this class
        "FUSED": [False, True]
    }
    basic_test_set = itertools.product(basic_test_set["BATCH"],
         basic_test_set["SEQLEN_Q_K"],
         basic_test_set["NUM_Q_K_HEADS"],
         basic_test_set["HEAD_SZ"],
         basic_test_set["DROPOUT_CAUSAL"],
         basic_test_set["FP8"],
         basic_test_set["FUSED"]
    )
    basic_set =  [
        pytest.param(*flatten(test))
        for test in basic_test_set
    ...
    extended_test_set = itertools.product(extended_test_set["BATCH"],
         ...
         extended_test_set["NUM_Q_K_HEADS"],
         extended_test_set["HEAD_SZ"],
         extended_test_set["DROPOUT_CAUSAL"],
         extended_test_set["FP8"],
         extended_test_set["FUSED"]
    )
    extended_set =  [
        pytest.param(*flatten(test), marks=pytest.mark.extended)
        for test in extended_test_set
    ...
    @pytest.mark.parametrize("BATCH, SEQLEN_Q, SEQLEN_K,NUM_Q_HEADS, NUM_K_HEADS, HEAD_SZ,DROPOUT, CAUSAL,  FP8, FUSED", all_test_params)
...
class TestMHABkwdVarlen:
    basic_test_set = {
        ...
        "HEAD_SZ": [8, 128],
        #"DROPOUT_CAUSAL": [[0.0, True], [0.0, False],[0.2, True], [0.2, False]], #TODO: Create separate tests for these in this class. #Debug Causal + Dropout. fails for seq >= 64
        "DROPOUT_CAUSAL": [[0.0, True], [0.0, False]], #TODO: Create separate tests for these in this class. #Debug Causal + Dropout. fails for seq >= 64
        #"FP8": [True, False], #TODO Create separate tests for these in this class #TODO Debug FP8
        "FP8": [False], #TODO Create separate tests for these in this class
        "FUSED": [False, True]
    }
