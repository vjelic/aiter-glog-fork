...
from aiter.ops.triton.moe_op_gelu import \
    moe_set_use_persistent_kernel as moe_set_use_persistent_kernel_gelu
...
@pytest.mark.parametrize("M, N, K, top_k, E", [# (64, 14336, 4096, 2, 8), # TODO: this causes OOM in torch_moe b_index = b[topk_ids]
                                               (16, 14336, 1, 2, 4),
                                               (4, 4, 8, 1, 2),
                                               (1, 14336, 128, 2, 4),
                                               (3, 14336, 128, 2, 4),
...
def test_fused_moe(M: int, N: int, K: int, top_k: int, E: int, routed_weight: bool, persistent: bool, silu_fused: bool, input_dtype, aux_dtype):
    ...
    a, b, triton_out, triton_out_silu, b_zp, a_scale, b_scale = \
        generate_moe_input(M, N, K, top_k, E, group_size, has_zp, input_dtype, aux_dtype)
    ...
    _triton_moe(a, b, triton_out_silu if silu_fused else triton_out, a_scale, b_scale, b_zp, topk_weights, topk_ids, sorted_token_ids, expert_ids,
                       num_tokens_post_padded, routed_weight, top_k, config, torch_to_tl_dtype[aux_dtype], fp8_w8a8, int8_w8a16, False)
    ...
    torch_out = torch_moe(a, b, torch_out, a_scale, b_scale, None, 0, topk_ids, topk_weights, routed_weight, sorted_token_ids, expert_ids,
                        num_tokens_post_padded, aux_dtype, fp8_w8a8, int8_w8a16, False)
    if silu_fused:
...
@pytest.mark.parametrize("M, N, K, top_k, E", [(1, 64, 128, 1, 2),
                                               (1, 64, 128, 2, 4),
                                               (4, 32, 64, 4, 16),
                                               (8, 96, 256, 2, 16)
                                               ])
...
def test_fused_moe_int4_w4a16(M: int, N: int, K: int, top_k:int, E: int,
    ...
    a, b, triton_out, triton_out_silu, b_zp, a_scale, b_scale = \
        generate_moe_input(M, N, K, top_k, E, group_size, has_zp, input_dtype=input_dtype, aux_dtype=dtype)
...
@pytest.mark.parametrize('persistent',[False, True])
def test_moe_fused_gelu(M: int, N: int, K: int, top_k: int, E: int, routed_weight: bool, persistent: bool, input_dtype, aux_dtype):
    moe_set_use_persistent_kernel(persistent)
    ...
    triton_moe_gelu(a, b, triton_out, a_scale, b_scale, topk_weights, topk_ids, sorted_token_ids, expert_ids,
                       num_tokens_post_padded, routed_weight, top_k, config, torch_to_tl_dtype[aux_dtype], fp8_w8a8, int8_w8a16)

    torch_out = torch.empty_like(triton_out)
    torch_out = torch_moe(a, b, torch_out, a_scale, b_scale, None, 0, topk_ids, topk_weights, routed_weight, sorted_token_ids, expert_ids,
                        num_tokens_post_padded, aux_dtype, fp8_w8a8, int8_w8a16, False, gelu=True)
