...
def test_flatmm(dtype, m, n, k):
    ...
    msg = f"[solin  perf] tflops: {tflops} ,dim: {str(dim):<20} dtype: {dtype}, torch avg: {avg_a:<8.2f} us, cktile avg: {avg_b:<8.2f} us, ck wpreshuffle avg: {avg_c:<8.2f} us,uplift: {avg_a/avg_b -1:<5.1%}"
    checkAllclose(a, b, msg="a,b: " + msg, rtol=1e-2, atol=0.01)
