...
import torch
import argparse
from aiter.ops.triton.moe_op import fused_moe as triton_moe, moe_set_use_persistent_kernel
import sys

def model_benchmark_configs(args):
    config_file = args.model_configs
    configs = get_model_configs(config_path=config_file, models="mistral,deepseek" if args.model == None else args.model)
    moe_configs = []
...
def fused_moe(M, N, K, top_k, E, routed_weight=False, dtype=torch.float16, int4_w4a16=False,
    ...
    if int4_w4a16:
        ...
        config = {
            ...
            "kpack": 1
        } #TODO using GROUP_SIZE_M > 1 results in accuracy issues
