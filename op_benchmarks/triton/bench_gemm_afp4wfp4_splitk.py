...
import torch
import triton
from aiter.ops.triton.gemm_afp4wfp4_splitk import gemm_afp4wfp4_splitk
from op_tests.triton_tests.test_gemm_afp4wfp4_splitk import generate_gemm_afp4wfp4_inputs
...
def run_benchmark(args):
    ...
    benchmark = triton.testing.Benchmark(
        ...
        styles=[('green', '-')],
        ylabel=ylabel, plot_name='GEMM MXFP4 x MXFP4 Benchmark', args={"metric": args.metric})

    @triton.testing.perf_report([benchmark])
    def bench_gemm_afp4wfp4_splitk(M, N, K, metric, provider):
        ...
        mem_read += x_scale.numel() * x_scale.element_size() + w_scale.numel() * w_scale.element_size()
        mem_write = (M * N) * 2 # TODO: Fix for c_dtype != bf16
        mem = mem_read + mem_write
        out = torch.empty(x.shape[0], w.shape[1], device=x.device, dtype=c_dtype)

        ms = triton.testing.do_bench(
            lambda: gemm_afp4wfp4_splitk(x, w, out, x_scale, w_scale, c_dtype),
            warmup=25, rep=100)
    ...
    bench_gemm_afp4wfp4_splitk.run(save_path=".", print_data=True)
