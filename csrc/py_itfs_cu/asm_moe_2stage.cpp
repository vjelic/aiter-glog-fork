...
void moe_stage1_fp8_blockscale_g1u1(torch::Tensor &out,               // [token_cnt, dim]
                              torch::Tensor &input,             // [token_cnt, dim] M,K
                              ...
                              std::optional<torch::Tensor> fc2_smooth_scale = std::nullopt) // [expert, 1, hidden_dim])
{
    ...
    if (out.dtype() == at::ScalarType::BFloat16 && hidden_dim % 256 == 0 && fc_scale_blkn == 128 && fc_scale_blkk == 128)
    {
        static MoeStage1_Kernel impl_128_novs("stage1_moe_fp8_blockscale_g1u1_subX128_subGU128", "stage1_moe_fp8_blockscale_g1u1_subX128_subGU128.co", 128);
        impl_ptr = &impl_128_novs;
    }
    else
        TORCH_CHECK(false, __func__, " Only support out dtype = bf16, hidden_dim % 256 = 0 and fc_scale_blkn and fc_scale_blkk is 128");
