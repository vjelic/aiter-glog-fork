...
import shutil
import torch
from gemm_a8w8_blockscale_wpreshuffle_common import (
    kernelInstance,
    kernels_list,
...
"""
        with open(
            os.path.join(
                self.working_path, "gemm_a8w8_blockscale_wpreshuffle_lookup.h"
            ),
            "w",
        ) as f:
            f.write(LOOKUP_head)
...
"""
        ...
        with open(
            os.path.join(
                self.working_path, "gemm_a8w8_blockscale_wpreshuffle_manifest.h"
            ),
            "w",
        ) as f:
