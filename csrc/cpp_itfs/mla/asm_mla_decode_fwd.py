...
def compile(hsaco_path: str, page_size: int, q_itemsize: int, kv_itemsize: int, num_kv_splits:int, v_head_dim:int, func_name: str = None):
    ...
    if not_built(func_name):
        ...
        return compile_template_op(src_template, MD_NAME, ["../utils.h", "../../include", triton_header], [triton_source], bin_size=bin_size, bin_data=bin_data, page_size=page_size, q_itemsize=q_itemsize, kv_itemsize=kv_itemsize, triton_header=triton_header, triton_kernel=triton_kernel, num_kv_splits=num_kv_splits, func_name=func_name)
    else:
