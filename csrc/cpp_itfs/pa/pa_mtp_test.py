...
def test_pa_mtp(
    ...
    out_asm_noquant = torch.empty_like(out_ref_noquant)
    total_query_len, num_heads, head_size = out_asm_noquant.shape
    # num_seqs = block_tables.size(0)
    PARTITION_SIZE = 256
    num_partitions = (max_seq_len + PARTITION_SIZE - 1) // PARTITION_SIZE
    tmp_output = torch.empty(
        size=(total_query_len, num_heads, num_partitions, head_size),
        dtype=out_asm_noquant.dtype,
    )
    ...
    torch.testing.assert_close(out_asm_noquant, out_ref_noquant, atol=1e-3, rtol=1e-3)
    # print(out_ref_noquant)
    # print(out_asm_noquant)
    ...
    # out_aiter_asm, us_aiter_asm = run_aiter(
    #     query,
    #     k_quant_,
