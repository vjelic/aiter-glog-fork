...
void paged_attention_ragged(
  ...
  std::string signature = fmt::format(
      "{gqa_ratio}_{head_size}_{npar_loops}_{dtype}_{kv_dtype}_{fp8_"
  ...
  std::string func_name = fmt::format("{}_{}", MD_NAME, hash_signature(signature));
  ...
  run_lib(func_name, folder, out_ptr, workspace_buffer_ptr, query_ptr, key_cache_ptr,
