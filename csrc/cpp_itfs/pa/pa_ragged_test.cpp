...
protected:
    ...
    void TearDown() override {
        ...
        if (query_ptr) hipFree(query_ptr);
        if (key_cache_ptr) hipFree(key_cache_ptr);
        if (value_cache_ptr) hipFree(value_cache_ptr);
        if (out_ptr) hipFree(out_ptr);
        if (workspace_buffer_ptr) hipFree(workspace_buffer_ptr);
        if (kv_indptr_ptr) hipFree(kv_indptr_ptr);
        if (kv_page_indices_ptr) hipFree(kv_page_indices_ptr);
        if (kv_last_page_lens_ptr) hipFree(kv_last_page_lens_ptr);
        if (k_scale_ptr) hipFree(k_scale_ptr);
        if (v_scale_ptr) hipFree(v_scale_ptr);
    }
...
TEST_F(PagedAttentionTest, BasicTest) {
    ...
    for (auto& val : key_cache_data) val = dist(gen);
    for (auto& val : value_cache_data) val = dist(gen);
    ...
    workspace_buffer_ptr = allocateAndInitGPU(num_seqs * num_heads * max_num_partitions * 2, std::vector<float>());
...
// TEST_F(PagedAttentionTest, WithAlibiTest) {
//     // Similar setup to BasicTest but with ALiBi slopes
//     std::vector<float> alibi_slopes_data(num_heads);
//     std::random_device rd;
//     std::mt19937 gen(rd());
//     std::uniform_real_distribution<float> dist(-1.0f, 1.0f);

//     for (auto& val : alibi_slopes_data) val = dist(gen);

//     // Allocate GPU memory for ALiBi slopes
//     float* alibi_slopes_ptr = allocateAndInitGPU(alibi_slopes_data.size(), alibi_slopes_data);

//     // Rest of the test setup similar to BasicTest
//     // ... (similar to BasicTest)

//     // Call paged_attention_ragged with ALiBi slopes
//     paged_attention_ragged(
//         std::nullopt,  // folder
//         query_ptr,
//         key_cache_ptr,
...
//     // Clean up
//     hipFree(alibi_slopes_ptr);
// }

// TEST_F(PagedAttentionTest, WithLastPageLensTest) {
//     // Similar setup to BasicTest but with last page lengths
//     std::vector<int> kv_last_page_lens_data = {8, 12};  // Last page lengths for each sequence

//     // Allocate GPU memory for last page lengths
//     kv_last_page_lens_ptr = allocateAndInitGPU(kv_last_page_lens_data.size(), kv_last_page_lens_data);

//     // Rest of the test setup similar to BasicTest
//     // ... (similar to BasicTest)

//     // Call paged_attention_ragged with last page lengths
//     paged_attention_ragged(
//         std::nullopt,  // folder
//         query_ptr,
//         key_cache_ptr,
...
//         kv_indptr_ptr,
//         kv_page_indices_ptr,
//         kv_last_page_lens_ptr,  // kv_last_page_lens_ptr
//         k_scale_ptr,
//         v_scale_ptr,
//         nullptr,  // fp8_out_scale_ptr
//         out_ptr,
