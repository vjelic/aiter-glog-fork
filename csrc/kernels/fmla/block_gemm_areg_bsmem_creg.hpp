...
CK_TILE_DEVICE bool IsDebugThreadBlock1(const int x = 0, const int y = 0, const int z = 0)
{
...
struct BlockGemmARegBSmemCReg
{
                                   ...
                                   const BBlockWindowTmp& b_block_window_tmp) const
    {
        ...
        // decltype(a_block_tensor)::aaa();
        // RJM DEBUG
        // __syncthreads();
        // if (IsDebugThreadBlock1()) {
        //     const auto debug_span = ck_tile::remove_cvref_t<decltype(a_block_tensor)>::get_distributed_spans();
        ...
        //                 a_block_tensor.get_tile_distribution(), i_j_idx);
        //             auto row_id = tile_idx.at(ck_tile::number<0>{});
        //             auto col_id = tile_idx.at(ck_tile::number<1>{});
        //             printf("[%d(%d), %d, %d, %d] a_block_tensor [%d, %d]: %f\n",
        //                 threadIdx.x, threadIdx.x / ck_tile::get_warp_size(), blockIdx.x, blockIdx.y, blockIdx.z,
        //                 row_id, col_id,
        //                 ck_tile::type_convert<float>(a_block_tensor[i_j_idx]));
        //         });
        //     });
        // }
    ...
    CK_TILE_DEVICE static constexpr auto MakeABlockTileDistribution()
    {
        ...
        constexpr auto a_block_outer_dstr_encoding = [&]()
        {
            ...
            else
            {
                ...
                return ck_tile::tile_distribution_encoding<
                    ck_tile::sequence<>,
                    ck_tile::tuple<ck_tile::sequence<MIterPerWarp, MWarp>, ck_tile::sequence<KIterPerWarp, KWarp>>,
                    ck_tile::tuple<ck_tile::sequence<1, 2>>,
                    ck_tile::tuple<ck_tile::sequence<1, 0>>,
                    ck_tile::sequence<1, 2>,
                    ck_tile::sequence<0, 1>>{};
                // return ck_tile::tile_distribution_encoding<
                //     ck_tile::sequence<KWarp>,
                //     ck_tile::tuple<ck_tile::sequence<MIterPerWarp, MWarp>, ck_tile::sequence<KIterPerWarp>>,
                //     ck_tile::tuple<ck_tile::sequence<1, 0>>,
                //     ck_tile::tuple<ck_tile::sequence<1, 0>>,
                //     ck_tile::sequence<1, 2>,
