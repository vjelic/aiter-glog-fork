...
// __global__ void flash_fwd_splitkv_mla_kernel_debug(
...
//                     reinterpret_cast<scalar_t*>(params.p_debug_value),
//                     ck_tile::make_tuple(kSizeDV, kBlockN),
//                     ck_tile::make_tuple(kBlockN, 1),  // strides
//                     ck_tile::number<kBlockN>{},
//                     I1);
...
//
//                 auto debug_p_dram = ck_tile::make_naive_tensor_view(
//                     reinterpret_cast<scalar_t*>(params.p_debug_p),
//                     ck_tile::make_tuple(kBlockM, kBlockN),
//                     ck_tile::make_tuple(kBlockN, 1),  // strides
//                     ck_tile::number<kBlockN>{},
//                     I1);
//                 auto debug_p_window = ck_tile::make_tile_window(
//                     debug_p_dram,
//                     ck_tile::make_tuple(kBlockM, kBlockN),
//                     {0, 0});
//                 // ck_tile::store_tile(debug_p_window, cast_tile<scalar_t>(p_compute));
//                 ck_tile::store_tile(debug_p_window, p_tile);
//
//                 auto debug_o_dram = ck_tile::make_naive_tensor_view(
//                     reinterpret_cast<scalar_t*>(params.p_debug_output),
//                     ck_tile::make_tuple(kBlockM, kSizeD),
//                     ck_tile::make_tuple(kSizeD, 1),  // strides
...
template <typename Traits, typename scalar_t, typename acc_t>
__device__ constexpr auto get_pv_block_gemm_vs()
{
    using GemmProblem = ck_tile::BlockGemmProblem<scalar_t,
                                                  scalar_t,
                                                  acc_t,
    ...
    const FlashMlaFwdParams params)
{
    ...
    const auto make_q_dram = [&](const scalar_t* data) {
        const auto q_dram_naive = make_naive_tensor_view<ck_tile::address_space_enum::global>(
            data,
            ck_tile::make_tuple(params.size_s, kSizeD),
            ck_tile::make_tuple(params.stride_s_q, 1),
            IPack,
    ...
    const auto make_k_dram = [&](const scalar_t* data, int32_t height) {
        const auto k_dram_naive = make_naive_tensor_view<ck_tile::address_space_enum::global>(
            data,
            ck_tile::make_tuple(height, kSizeD),
            ck_tile::make_tuple(params.stride_s_k, 1),
            IPack,
        ...
        return ck_tile::pad_tensor_view(
            k_dram_naive,
            ck_tile::make_tuple(params.page_block_size, kSizeD),
            ck_tile::sequence<true, false>{});
    };

    auto k_page_block_navigator_func = [&, i_nhead_k_ = i_nhead_k](int32_t i_batch_, int32_t seqlen_kv_end_) {
        const auto* block_indices =
            reinterpret_cast<const int32_t*>(params.p_block_table) +
            i_batch_ * params.block_table_batch_stride;
        const int32_t num_blocks =
        ...
        return ck_tile::make_page_block_navigator<const scalar_t, 0>(
            ...
            num_blocks,
            params.page_block_size, // page_size
            make_k_dram(nullptr, params.page_block_size),
            // make_k_dram(nullptr, params.page_block_size));
            make_k_dram(nullptr,
                        (seqlen_kv_end_ - (num_blocks - 1) * params.page_block_size))); //TODO: fix the tail block size in kvcache
    };
    ...
    auto make_lse_dram_window = [&] (int i_batch_) {
        const auto lse_dram = make_naive_tensor_view<ck_tile::address_space_enum::global>(
            reinterpret_cast<acc_t*>(params.p_softmax_lse) + i_batch_ * params.size_s,
            ck_tile::make_tuple(params.size_s),
            ck_tile::make_tuple(1),
            IPack,
            I1);
        return make_tile_window(lse_dram, lse_acc_dram_window_lengths, {i_m0});
    };
    ...
    auto make_o_acc_dram_window = [&, i_nhead_ = i_nhead, i_block_m_ = i_block_m](int i_batch_, int i_split_) {
        ...
        const auto o_acc_dram = make_naive_tensor_view<ck_tile::address_space_enum::global>(
            o_acc_ptr,
            ck_tile::make_tuple(params.size_s, kSizeDV),
            ck_tile::make_tuple(kSizeDV, 1),
            IPack,
    ...
    auto make_o_dram_window = [&] (int i_batch_) {
        const auto o_dram = make_naive_tensor_view<ck_tile::address_space_enum::global>(
            reinterpret_cast<scalar_t*>(params.p_output) + i_batch_ * params.stride_b_o,
            ck_tile::make_tuple(params.size_s, kSizeDV),
            ck_tile::make_tuple(params.stride_s_o, 1),
            IPack,
            I1);
		return make_tile_window(o_dram, o_dram_window_lengths, {i_m0, 0});
    };
...
#ifdef zz_debug_block_gemm_ks
    auto gemm_1 = get_pv_block_gemm_vs<Traits, scalar_t, acc_t>();
#else
    auto gemm_1 = get_kv_block_gemm<Traits, scalar_t, acc_t>();
#endif
    auto o_acc = gemm_1.MakeCBlockTile();
    ...
    for (int32_t i_batch = begin_batch_idx; i_batch <= end_batch_idx; ++i_batch)
    {
        ...
        auto k_dram = make_k_dram(nullptr, params.page_block_size);
        auto k_page_block_navigator = k_page_block_navigator_func(i_batch, split_seqlen_k_end);
        ...
        if ((tidx == DEBUG_TID || tidx == 0) && blockIdx.x == 0 && blockIdx.y == 0 && (blockIdx.z == 0))
        {
            ...
            // TODO: need check k pad
            auto debug_k_page_block_navigator = k_page_block_navigator_func(i_batch, split_seqlen_k_end);
        ...
        auto k_st_lds_window =
            make_tile_window(k_lds,
                ck_tile::make_tuple(ck_tile::number<kBlockN>{},
                                    ck_tile::number<kSizeD>{}), {0, 0});
        auto k_st_tail_lds_window =
            make_tile_window(k_lds,
                ck_tile::make_tuple(seqlen_k - (n_block_max - 1) * kBlockN,
                                    ck_tile::number<kSizeD>{}), {0, 0});
        ...
        if (i_block_n % 2 == 1)
        {
            ...
            move_tile_window(v_ld_lds_window, {0, v_ld_lds_offset});
#ifdef ZZDebug
        ...
        auto k_block_tile = load_tile(k_dram_window_tail);
        store_tile(k_st_lds_window, k_block_tile);

        auto [i_page_block_k, k_dram_window] = k_page_block_navigator.make_tile_window(
            k_dram_window_lengths, {(n_block_max - 2) * kBlockN, 0}, k_dram_tile_distribution);
...
// #endif
            ...
            else
            {
                ...
                sweep_tile_span(o_spans[I1], [&](auto idx1) {
                    ...
                    sweep_tile_span(o_spans[I1], [&](auto idx0) {
                        constexpr auto i_j_idx = ck_tile::make_tuple(idx0, idx1);
// #ifdef ZZDebug
//                         const auto tile_idx = get_x_indices_from_distributed_indices(o_acc.get_tile_distribution(), i_j_idx);
//                         auto row_id = tile_idx.at(ck_tile::number<0>{});
...
// #endif
        ...
        if (NoSplit)
        {
            auto lse_dram_window = make_lse_dram_window(i_batch);
            store_tile(lse_dram_window, lse_acc);
        }
        else
        {
        ...
        if (NoSplit)
        {
            auto o_dram_window = make_o_dram_window(i_batch);
            store_tile(o_dram_window, ck_tile::cast_tile<scalar_t>(o_acc));
        }
        else
        {
