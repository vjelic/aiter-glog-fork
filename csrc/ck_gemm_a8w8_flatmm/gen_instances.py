...
import argparse
import shutil
from flatmm_ck_common import kernelInstance, kernels_list, default_kernels_dict


""" 

flatmm_ck instance gen 

"""


class flatmm_ck_codegen:
...
"""     
        ...
        // Run kernel instance.
        return flatmm_ck_impl<DDataType, EDataType, FlatmmInstance>(XQ, WQ, x_scale, w_scale, Y);
""" 
        if self.istune:
...
#endif // USE_ROCM
"""
        with open(os.path.join(self.working_path, "flatmm_ck_lookup.h"), "w") as f:
            f.write(LOOKUP_head)
...
"""
        ...
        with open(os.path.join(self.working_path, "flatmm_ck_manifest.h"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--tune_file",
        default="aiter/configs/flatmm_tuned.csv",
        required=False,
        help="tune_file include the result after run flatmm_ck_tune.py"
    )
    ...
    codegen = flatmm_ck_codegen(args.working_path, args.tune)
