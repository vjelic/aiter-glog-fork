...
CK_MOE_STAGE{Stage}_GEMM_BLOCKSCALE_DEFINE({BlockSize}, {MPerBlock}, {NPerBlock}, {KPerBlock}, {MWaves}, {NWaves}, {MNPerXDL})
"""
...
LOOKUP_template = """
       {{"{kernel_tag}",                                                                                                       \\
        moe_stage{stage}_gemm_blockscale<{A0DataType}, {B0DataType}, {AccDataType}, {EDataType}, {CDEElementOp}, {BLOCKSIZE}, {MPerBlock}, {NPerBlock}, {KPerBlock}, {MWaves}, {NWaves}, {MNPerXDL}, {Nswizzle}, {PerTensorQuant}>}},                       \\"""
...
class ck_moe_2stage_gemm_codegen:
    ...
    def generate_instance_and_lookUpTable(self):
        ...
        f_lookUpTable = os.path.join(self.working_path, "gemm_moe_2stages_blockscale_lookup.h")
        ...
        with open(f_lookUpTable, "w") as f_lookup:
            ...
            for idx, row in df_tuned.iterrows():
                ...
                with open(f_instance, "w") as f_ins:
                    ...
                    args =  r"stage([12])_([a-zA-Z0-9]+)_([a-zA-Z0-9]+)_([a-zA-Z0-9]+)_([a-zA-Z_0-9x\d+]+)_(\d+)x(\d+)x(\d+)x(\d+)_(\d+)x(\d+)_(\d+)_([a-zA-Z0-9]+)_Nswizzle([01])"
